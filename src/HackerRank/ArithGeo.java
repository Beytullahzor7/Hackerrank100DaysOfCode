...
public class ArithGeo {
    ...
    public static String arithGeo(int[] arr) {
        Arrays.sort(arr);
        int firstIndex = arr[0], secondIndex = arr[1], lastIndex = arr[arr.length - 1], beforeLastIndex = arr[arr.length - 2];

        if (secondIndex - firstIndex == (lastIndex - beforeLastIndex)) return "Arithmetic";
        else if (secondIndex / firstIndex == (lastIndex / beforeLastIndex)) return "Geometric";
